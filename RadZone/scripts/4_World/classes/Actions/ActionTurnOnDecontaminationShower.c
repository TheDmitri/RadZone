class ActionTurnOnDecontaminationShower: ActionInteractBase
{
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
	{
		...
		if ( targetObject )
		{
			...
			if (shower.FindAttachmentBySlotName("CanisterGasoline") != NULL)
			{
				EntityAI m_Tank = shower.FindAttachmentBySlotName("CanisterGasoline");
				ItemBase m_Tank_IB = ItemBase.Cast(m_Tank);

				if ( m_Tank_IB.GetLiquidType() != LIQUID_GASOLINE && m_Tank_IB.GetQuantity() > 0 && !shower.GetCompEM().IsWorking())
				{
					return true;
				}
				else false;
