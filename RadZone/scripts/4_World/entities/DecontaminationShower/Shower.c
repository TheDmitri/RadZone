...
{
  static const string			SHOWER_SOUND 				= "rz_Shower_SoundSet";
  ...
  vector m_ParticleLocal1Pos = Vector(-0.363,0.580,-0.033);

  vector m_ParticleLocal2Pos = Vector(-0.363,0.866,-0.033);

  vector m_ParticleLocal3Pos = Vector(-0.363,1.253,-0.033);

  vector m_ParticleLocal4Pos = Vector(-0.363,1.543,-0.033);

  vector m_ParticleLocal5Pos = Vector(0.101,1.973,-0.033);

  vector m_ParticleLocal6Pos = Vector(0.391,1.973,-0.033);
  ...
  void PlayParticle()
  {
    if (GetGame().IsClient() || !GetGame().IsMultiplayer()){
      ...
      m_Particle4Efx = Particle.PlayOnObject(ParticleList.SHOWER90, this,m_ParticleLocal4Pos, Vector(0,0,0), false);
  ...
  void CleanPlayer(PlayerBase player)
  {
	...
	override void OnWorkStop()
	{
		super.OnWorkStop();
    #ifdef RZDEBUG
    GetRZLogger().LogInfo("Shower_OnWork Stop"+"current energy: "+GetCompEM().GetEnergy().ToString());
    #endif
		if ( GetGame().IsClient()  ||  !GetGame().IsMultiplayer() )
		{
        StopSound();
  ...
  void StartSound()
	{
		...
		m_SoundLoopStartTimerShower.Run(0.4, this, "StartLoopSound", NULL, true);
	}
  ...
  void CanStartShower()
	{
    EntityAI m_Tank;
    m_Tank = this.FindAttachmentBySlotName("CanisterGasoline");
