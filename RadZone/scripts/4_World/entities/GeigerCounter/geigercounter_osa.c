// Script File
class Geigercounter_Osa extends ItemBase
{
	...
	int GetLevelValue(int nbradgiven)
	{
			...
			if(nbradgiven <= 20)return 1;
			if(nbradgiven <= 40)return 2;
			if(nbradgiven <= 60)return 3;
			if(nbradgiven <= 80)return 4;
			if(nbradgiven <= 100)return 5;
			if(nbradgiven <= 120)return 6;
			return 0;
	}
